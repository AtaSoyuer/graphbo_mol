...
class UCBalg:
    def __init__(self, qm9_data, qm9_val_data, init_grad_data, init_grad_loader, dataset_loader, val_dataset_loader, mean: float, std: float,
                 ...
                 alpha=0.25, gamma=2.0, large_scale=False, bernoulli_selection=False, ucb_wo_replacement=False, pool_top_means=False, small_net_var = False, initgrads_on_fly = False, oracle=False, select_K_together=False, select_K=5, path: Optional[str] = None, **kwargs):
        ...
        self.U = torch.zeros((self.num_net_params,)).to(self.device)
...
class GnnUCB(UCBalg):  # Our main method
    ...
    def __init__(self, qm9_data, qm9_val_data, init_grad_data, init_grad_loader, dataset_loader, val_dataset_loader,
                 ...
                 alpha=0.25, gamma=2.0, large_scale=False, bernoulli_selection=False, ucb_wo_replacement=False, pool_top_means=False, initgrads_on_fly = False, oracle=False, select_K_together=False, select_K=5, random_state = None, path: Optional[str] = None, **kwargs):
        super().__init__(qm9_data=qm9_data, qm9_val_data=qm9_val_data, init_grad_data=init_grad_data, init_grad_loader=init_grad_loader, dataset_loader=dataset_loader, val_dataset_loader=val_dataset_loader, net=net, feat_dim=feat_dim, dim=dim, num_mlp_layers=num_mlp_layers, alg_lambda=alg_lambda, verbose = verbose, #INHERITS THE BASE UCB ALGO CLASS
                         ...
                         select_K_together=select_K_together, select_K=select_K, oracle=oracle, path=path, **kwargs)
    ...
    def select(self): #COMPUTE POSTERIOR MEAN AND VARIANCES FOR ALL POSSIBLE CANDIDATES IN ACTION SET (WHERE SELF.U IS COMPUTED FROM PTS ADDEDF SO FAR), AND SELECTT CANDIDATE BASED ON UCB
        ...
        if self.no_var_computation:
            ...
            post_vars = torch.zeros(len(indices)).to(self.device)
            ...
            if self.online: #VECTORIZE COMPUTATIONS TO BE FASTER!!!
                ...
                if self.initgrads_on_fly:
                    g_vectors = torch.stack(self.init_grads_on_demand(indices)).to(self.device).to(dtype=self.dtype)
                else:
                    g_vectors = torch.stack(self.init_grad_list)[indices].to(self.device).to(dtype=self.dtype)

                if self.initgrads_on_fly:
                    g_vectors_observed = torch.stack(self.init_grads_on_demand(self.data['graph_indices'])).to(self.device).to(dtype=self.dtype)
                else:
                ...
                if len(self.data['graph_indices']) <= 1:
                    ...
                    post_vars = (self.alg_lambda)**(-0.5)*torch.sqrt(torch.sum( g_vectors * g_vectors_observed, dim=1 ) / self.neuron_per_layer - torch.sum(kx_t_matrix * self.U * kx_t_matrix, dim=1))
                else:
                    #print('Computing variances based on Mahalanobis Expression')
                    kx_t_matrix = (torch.matmul(g_vectors, g_vectors_observed.t()).to(dtype=self.dtype) / self.neuron_per_layer)
                    ...
                    #kx_t = (torch.matmul(g, torch.stack(self.init_grad_list)[:,self.data['graph_indices']].flatten(start_dim = 1)) / self.neuron_per_layer)
                    #print(kx_t.shape)
                    post_vars = (self.alg_lambda)**(-0.5)*torch.sqrt(torch.sum( g_vectors * g_vectors, dim=1 ) / self.neuron_per_layer - torch.sum(torch.matmul(kx_t_matrix, self.U) * kx_t_matrix, dim=1))
        ...
        if self.no_var_computation:
            ...
            if self.alternative:
                ...
                if len(self.data['graph_indices']) <= 1:
                    #print('Computing variances based on Mahalanobis Expression')
                    kx_t = (torch.matmul(g, g_vectors_observed.to(self.device).t())).to(dtype=self.dtype) / self.neuron_per_layer
                    post_var = (self.alg_lambda)**(-0.5)*torch.sqrt(torch.sum( g * g, dim=1).to(dtype=self.dtype) / self.neuron_per_layer - torch.sum(kx_t * self.U * kx_t, dim=1))
                    ...
                    #print(torch.stack(self.init_grad_list)[self.data['graph_indices']].t().shape)
                    #print(kx_t.shape)
                    post_var = (self.alg_lambda)**(-0.5)*torch.sqrt(torch.sum( g * g, dim=1).to(dtype=self.dtype) / self.neuron_per_layer - torch.sum(torch.matmul(kx_t, self.U) * kx_t, dim=1))
                    #print('Post var:', post_var)
                    #print('Post_var:',post_var.shape)
                    return post_var
            else:
