...
import sys
import json
from algorithms_gp_mp import SS_alg, SS_GP
import time
import numpy as np
import torch
...
import itertools
from supervised_pretraining import SupervisedPretrain_GP_gpy
from supervised_pretraining import GP_gpy
import gpytorch
import csv
...
def main(args):
    ...
    if args.laplacian_features:
        if args.addpe_features:
            transform = T.Compose([T.AddRandomWalkPE(walk_length=args.walk_length, attr_name=None) ,T.AddLaplacianEigenvectorPE(k=args.laplacian_k, attr_name=None, is_undirected=True), MyTransform(target), Complete(), T.Distance(norm=False)])
            args.feat_dim += 1 
            args.feat_dim += args.laplacian_k    
        else:
            transform = T.Compose([T.AddLaplacianEigenvectorPE(k=args.laplacian_k, attr_name=None, is_undirected=True), MyTransform(target), Complete(), T.Distance(norm=False)])
            args.feat_dim += args.laplacian_k         
    else:
        if args.addpe_features:
            transform = T.Compose([T.AddRandomWalkPE(walk_length=args.walk_length, attr_name=None), MyTransform(target), Complete(), T.Distance(norm=False)])
            args.feat_dim += 1
        else:
            transform = T.Compose([MyTransform(target), Complete(), T.Distance(norm=False)])

    if args.adddegree_features:
       t_deg = T.Compose([T.LocalDegreeProfile(),])
       t_deg.extend(transform)
       transform = t_deg
    ...
    if args.load_pretrained:
        pretraining_alg = SupervisedPretrain_GP_gpy(target=args.reward, input_dim = args.feat_dim, reward_plot_dir = args.reward_plot_dir, num_epochs = args.num_epochs, \
        ...
        GP_pretrained = pretraining_alg.train_loop()
        pretraining_alg.print_and_plot()
        pretrain_set, pretrain_labels = pretraining_alg.return_pretrain_set()

    else:

        raise NotImplementedError
    
    print('Finished Supervised Pretraining')
    ...
    if args.load_pretrained:
        ...
        val_loader = DataListLoader(val_dataset, batch_size=25, shuffle=False)
        train_loader = DataListLoader(train_dataset, batch_size=25, shuffle=False)
        init_grad_loader = DataListLoader(train_dataset, batch_size=1, shuffle=False)
    ...
    learner = SS_GP(qm9_data=train_dataset, qm9_val_data=val_dataset, init_grad_data=train_dataset, init_grad_loader=init_grad_loader, dataset_loader=train_loader, action_domain = graph_data, num_actions = args.num_actions,
                         val_dataset_loader=val_loader, mean=mean, std=std, feat_dim=args.feat_dim, num_nodes=args.num_nodes, dim=args.dim, batch_GD=args.batch_GD, num_mlp_layers=args.num_mlp_layers, alg_lambda=args.alg_lambda,
                         exploration_coef = args.exploration_coef, lr=args.lr, random_state = algo_rds, reward = args.reward, net = GP_pretrained, load_pretrained=args.load_pretrained, subsample=args.subsample, subsample_method=args.subsample_method, 
                         verbose = args.runner_verbose, subsample_num=args.subsample_num, pool=args.pool, pool_num=args.pool_num, large_scale=args.large_scale, bernoulli_selection=args.bernoulli_selection, ucb_wo_replacement=args.ucb_wo_replacement, 
                         pool_top_means=args.pool_top_means, select_K_together=args.select_K_together, select_K=args.select_K, batch_size = args.batch_size, pretrain_set = pretrain_set, pretrain_labels = pretrain_labels, thompson_sampling = args.thompson_sampling)

    #Initialize the algortihm & the model as above width learner 
    ...
    def run_select(dummy = None): #COMPUTE POSTERIOR MEAN AND VARIANCES FOR ALL POSSIBLE CANDIDATES IN ACTION SET (WHERE SELF.U IS COMPUTED FROM PTS ADDEDF SO FAR), AND SELECTT CANDIDATE BASED ON UCB
        ...
        if args.alternative:
            ...
            learner.GP_model.eval()
            learner.GP_model.likelihood.eval()

            with torch.no_grad(), gpytorch.settings.fast_pred_var():
                    
                observed_pred = learner.GP_model.likelihood(learner.GP_model(learner.x_train[indices,:].to(device)))
                lower, upper = observed_pred.confidence_region()
                #print('upper_conf_bound:', upper.shape)
                #print('observed_preds:', observed_pred.shape)

            post_vars = (((upper - lower)/2.0)**2).to(learner.device_select)
            print('Post_vars:', post_vars.shape)
        
            post_means = learner.data['means'][indices].to(learner.device_select)
            print('Means:', post_means.shape)
    ...
    learner.create_train_val_data_matrices()

    t0 = time.time()

    print('Time for creating data matrices:', t0-t_0_1)
    ...
    while t < args.T - 1: #T is the total number of steps that the algortihm runs for, see paper
       ...
       T0 DETERMINES WHETHER BATCH_SIZE IS ACTIVE OR NOT, THUS WHETHER WE NEED RTO PARALLELIZE!!
        ...
        if t < args.T0: 
            ...
            learner.add_data_prll([action_t] if not isinstance(action_t, list) else action_t, [observed_reward_t] if not isinstance(observed_reward_t, list) else observed_reward_t)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--addpe_features', default=False, action='store_true', help='Add Random Walk Positional Encoding')
    parser.add_argument('--walk_length', type=int, default=3)

    parser.add_argument('--adddegree_features', default=False, action='store_true', help='Add Local Degree Profile Features')
