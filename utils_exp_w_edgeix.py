...
from graph_env.graph_generator_w_edgeix import Graph
...
def dataset_to_graphdata(dataset, args):
    ...
    graph_edge_indices = list(dataset['edge_indices'])[0]
    if args.synthetic:
    ...
    graph_data = [Graph(dim_feats=feat_dim, num_nodes=num_nodes,adj_mat=np.array(adj_mat),feat_mat=np.array(feat_mat), edge_index=np.array(edge_index), synthetic=args.synthetic ) for adj_mat, feat_mat, edge_index in zip(graph_connections,graph_features, graph_edge_indices)]
...
def generate_run_commands(command_list, num_cpus=1, dry=False, n_hosts=1, mem=5000, long=False,
