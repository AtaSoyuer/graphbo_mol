...
def main(args):
    ...
    for r in [0,1,2,3,4,5,6,7,11,12]: #[0,1,2,3,4]
        ...
        for config in configurations:
            ...
            sub_df = sub_df.loc[sub_df['thompson_sampling'] == False]
            ...
            graph_rewards = np.array([np.squeeze(np.array(percentages)) for percentages in sub_df['graph_rewards']])[0] * std + mean
            means = np.array([np.squeeze(np.array(percentages)) for percentages in sub_df['means']])[0] * std + mean
