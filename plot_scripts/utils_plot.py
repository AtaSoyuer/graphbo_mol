...
def plt_cumulative_rew(t: int, net: str, print_every: int, rewards: list, rewards_bp: list,
               plot_vars: bool = True, avg_vars: Optional[list] = None, pick_vars_all: Optional[list] = None):
    if plot_vars:
        fig, ax = plt.subplots(nrows=2, figsize=(8, 8))
        # plot regret
        ax[0].plot(rewards, color='#9dc0bc', label=net+'-UCB')
        ax[0].plot(rewards_bp, color='#c6ecae', label=net + 'UCB, Best Point Cumulative Rewards')
        ax[0].set_title(f'Cumulative Rewards, t = {t}')
        ax[0].legend()
        ax[0].set(xlabel='t', ylabel=r'$Reward_t$')
        ...
        plt.figure()
        # plot regret
        plt.plot(rewards, color='#9dc0bc', label=net+'UCB, Rewards')
        plt.plot(rewards_bp, color='#c6ecae', label=net+'UCB, Best Point Rewards')
        plt.title(f'Cumulative Rewards (BP), t = {t}')
        plt.legend()
        plt.xlabel(r'$t$')
        plt.ylabel(r'$Reward_t$')
