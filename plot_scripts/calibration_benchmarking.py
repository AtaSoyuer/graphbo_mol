...
def main(args):
    ...
    for r in ['5','6','7','11','12']: #['0','1','2','3','4',]: #['5','6','7','11','12']: #[0,1,2,3,4]
        print('reward:', r)
        reward = r
        #df_net = df_full.loc[df_full['reward'] == r]
        df_net = df_full
        #print(df_net['algorithm'])
        configurations = [config for config in
                        ...
                        df_net['algorithm'] if not all(z == config[0] for z in config[1:])]
        ...
        for config in configurations:
            ...
            curve_name_prepends = [r'$\bf{GNN-SS-UCB} $', r'$\bf{GNN-SS-TS} $', r'$\bf{SOBER} $']
            #print(sub_df)
            ...
            if (sub_df['algorithm'] == 'ucb').all():
                if (sub_df['thompson_sampling'] == True).all():
                    sub_df = sub_df.loc[sub_df['reward'] == int(r)]
                ...
                sub_df = sub_df.loc[sub_df['reward'] == r]
            ...
            axes_cal[row_counter][0].plot(alphas, np.mean(coverages, axis=0), linestyle[net+'_UCB'],  label = curve_name, color = clr, linestyle = linestyles[counter%13])
            axes_cal[row_counter][0].fill_between(alphas, np.mean(coverages, axis=0)-np.std(coverages, axis=0),
                                np.mean(coverages, axis=0)+np.std(coverages, axis=0), alpha=0.2,
                                color = clr)
            axes_cal[row_counter][0].plot([0.0,1.0], [0.0,1.0], color='black', linewidth=0.5, alpha=0.1)
            axes_cal[row_counter][0].grid(alpha=0.8)
            #print(alpha_sharpness_str)
            axes_cal[row_counter][1].bar(alpha_sharpness_str , np.mean(avg_widhts, axis=0), label = curve_name, color = clr,  width=0.3, alpha=0.3)
            axes_cal[row_counter][1].errorbar(alpha_sharpness_str, np.mean(avg_widhts, axis=0), yerr=0.5*np.std(avg_widhts, axis=0), fmt='o', markersize=4, capsize=8, color = clr)
        ...
        axes_cal[row_counter][0].set_title(r'$QM9:$'+rewards_names[int(reward)],)#fontsize=20)
        axes_cal[row_counter][1].set_title(r'$QM9:$'+rewards_names[int(reward)],)#fontsize=20)
        ...
        axes_cal[row_counter][0].set(xlabel=r'$\alpha$', ylabel=r'Calibration of Confidence Intervals')
        axes_cal[row_counter][1].set(xlabel=r'$\alpha$', ylabel=r"Sharpness of Confidence Intervals")
