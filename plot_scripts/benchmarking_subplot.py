...
def main(args):
    ...
    plt.rcParams.update(bundles.icml2022(ncols=3,nrows=1,tight_layout=True))
    #fig_reg, axes_reg = plt.subplots( ncols = 1, nrows=1, (12,9))
    fig_rew_n_topk, axes_rew_n_topk = plt.subplots( ncols = 3, nrows=1,figsize=(15,4))
    ...
    for net in ['GNN']:
        ...
        for config in configurations:
            ...
            curve_name_prepends = [r'$\bf{GNN-SS-UCB} $', r'$\bf{GNN-SS-UCB(TRANSFER)} $', r'$\bf{SS-RANDOM} $', r'$\bf{SS-ORACLE} $']
            ...
            # if (sub_df['pretrain_model_name'] == 'nnconv_reward3n4_8000samples_100ep').all():
            # #if (sub_df['no_var_computation'] == True).all():
            #     dicts[keys[1]] = simple_regret.tolist()

            # elif (sub_df['oracle'] == True).all():
            #     dicts[keys[2]] = simple_regret.tolist()

            # elif (sub_df['T2'] == sub_df['T']).all():
            #     dicts[keys[3]] = simple_regret.tolist()

            # else:
            #     dicts[keys[0]] = simple_regret.tolist()

            
            if curve_name == curve_name_prepends[3]:

                axes_rew_n_topk[0].plot(np.mean(simple_regret, axis=0), linestyle[net+'_UCB'],  label = curve_name, color = clr, linestyle = lst)
                axes_rew_n_topk[0].fill_between(np.arange(configs['T']-1), np.mean(simple_regret, axis=0), MAX_REWARD, alpha=0.2,
                                    color = clr)
                axes_rew_n_topk[0].axhline(y = MAX_REWARD, color = 'dimgray', linestyle = '-', linewidth=2)
            ...
            axes_rew_n_topk[1].plot(top_k_percentages , np.mean(top_k, axis=0), linestyle[net+'_UCB'],  label = curve_name, color = clr, linestyle = lst, marker='o', markersize=4)
        ...
        #axes_rew_n_topk[2].set_title(r'$QM9:$'+rewards_names[reward],)#fontsize=20)
        #axes_rew_n_topk[1].set_title(r'$QM9:$'+rewards_names[reward],)#fontsize=20)
        #axes[row_counter][col_counter].set_ylabel(r'$R_{BP,T}$')
        axes_rew_n_topk[0].set(xlabel=r'Online Oracle Calls', ylabel=r'$top\_1$')
        axes_rew_n_topk[1].set(xlabel=r'Log-TopK Percent Samples Evaluated', ylabel=r"Ratio of Top-K Samples Acquired")

        axes_rew_n_topk[2].set(xlabel=r'Online Oracle Calls', ylabel=r'$R_t$')
        #axes_rew_n_topk[0].set_title(r'$QM9:$'+rewards_names[reward],)
        col_counter += 1

    lines_labels = [ax.get_legend_handles_labels() for ax in [axes_rew_n_topk[0], axes_rew_n_topk[1]]] #axes_rew_n_topk[2]]]
    lines, labels = [sum(lol, []) for lol in zip(*lines_labels)]
    #print(lines)
    #print(labels)
    fig_rew_n_topk.legend(lines[:4],labels[:4], loc ='lower center',bbox_to_anchor=(0.5, -0.1), fancybox = True, ncol = 8, prop = { "size": 12 } )
