...
import sys 
import os
sys.path.append(os.path.abspath("/local/bsoyuer/base_code/graph_BO/graph_env"))
from kernel_function import GntkJax
from graph_generator import generate_graph_data

'''
I THINK THIS SCRIPT UTILIZES KERNEL AND GRAPH GENERATOR SCRIPTS IN THIS DIRECTORY
TO DEFINE FUNCTIONS THAT BUILD SYNTHETIC GRAPH DATAPOINTS WITH AND REWARDS SAMPLED
ARTIFICIALLY FROM GP POSTERIR WITH COMPUTED GNTK AS MENTIONED IN THE PAPER.
'''
...
class Reward:
    def __init__(self, evidence_size: int, num_nodes: int, dim_feats: int, num_mlp_layers: int, noise_var: float,
        ...
        # prepare the evidence' i.e data, to calculate the posterior based on
        ...
        # generate the kernel matrix object
        self.kernel = GntkJax(num_nodes=num_nodes, num_mlp_layers=num_mlp_layers)

        #prepare the prior based on the below link
        ...
        if evidence_reward is not None:
            ...
            if random_evidence == 'prior':
                ...
                f_prior = jnp.dot(self.phi_ev, beta) #Also based on above link
    ...
    def generate_dataset(self, domain: list): #Create rewards only here I guess???
    ...
    def evaluate(self, idx_list: list, noisy: bool): #Add aleotoric noise to sampled rewards
...
if __name__ == '__main__':
    ...
    plt.plot(labels) #Plot sampled rewards (noisy) for the test set(after seeing
    #datasize many evidence sampels, calculate GNTK Posterior and sample test_size many rewards)    #plt.show()
