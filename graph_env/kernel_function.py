...
''' I THINK USED SOLELY FOR SYNTHETIC REWARD GENERATION FROM GP WITH GNTK?????'''
class GntkDu(object): 
    """
    implement the Graph Neural Tangent Kernel
    """
    ''' I THINK THE RECURSIVE IMPLEMENTATION FROM PAPER DU. ET AL. UNUSED???'''
    ...
    @staticmethod #I THINNK RECURSIVE, LAYER BY LAYER COMPUTATIONS FOR GPNN KERNEL COMPUTATION.
    #WHICH IS RECURSIVELY FIRST REQUIRED TO COMPUTE GNTKS OF THAT LAYER AS YOU REMEMBER
    #REMEMBER SUCH EXPRESSIONS ALSO CAME OUT IN YOUTUBE LECTURE FOR NTKS, ONLY NOW, THE COVARIANCE
    #MATRICES IN EACH LAYER ARE COMPUTED IN MATRIX FORM BY VECTOR OPERATIONS
    ...
    def _adj(cov_matrix_s, adj_block, n1, scale_mat, n2=None):
        ...
        S: the covariance I THINK THEY MEAN Cov_Matrix_S ????
        adj_block: the adjacency relation
        n1: number of vertices
        scale_mat: scaling matrix
        """
        if n2 is None:
            n2 = n1 #Regular Matrix Multiplication by '.dot'
...
class GntkJax:
    """
    I THINK THIS IS THE GNTK COMPUTATION USED AS EXPLAINED IN PAPER APPENDIX D.
    THEY USED NTK LIBRARY TO COMPUTE NTKS AND USED THE DERIVATIONS IN THE PAPER TO
    COMPUTE THE ACTUAL GNTK???
    """
    def __init__(self, num_mlp_layers: int, num_nodes: int): #Define a usual NN
        #the width here does not matter' since I thnik all the computations are in infinite widht limit by NTK Lib
        module = stax.Dense(100)
        layers = []
        for _ in range(num_mlp_layers):
            layers += [module, stax.Relu()] #REGULAR, wide dense hidden layers
        layers += [stax.Dense(1)] #Output Layer

        _, _, kernel = stax.serial(*layers) #Stax' aka NTK LIB yields the computed NTK Kernel For
        #the above NN Architecture which is related to our GNN Architecture
        self.base_kernel = jit(kernel, static_argnums=(2,)) #DEFINE BASE KERNEL ABSED ON????
    ...
    def gntk(self, g1: Graph, g2: Graph):
        ...
        node_pairs = [] #Iterable List of pairwise node features
        ...
        return np.sum(np.array(ntks))/(self.num_nodes**2) #THE DERIVED EXPRESSION FOT GNTKS IN THE
        #PAPER AS SUM OF NTKS????
    ...
    def get_kernel_matrix(self, graphs1: list, graphs2: Optional[list] = None):
        if graphs2 is None:
            ...
            for t, v in zip(graph_pairs, results):
                gram[t[0], t[1]] = v
            return gram #COMPUTE THE GNTK IN KERNEL MATRIX FORM FOR THE GIVEN PAIRS OF GRAPHS
