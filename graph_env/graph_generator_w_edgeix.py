...
class Graph: #USED IN UTILS AND NETS!!!
    def __init__(self, num_nodes: int, dim_feats: int, sampling_mode: str = 'binomial', synthetic: Optional[bool] = True,
                 adj_mat: Optional[np.ndarray] = None, edge_prob: Optional[float] = None, num_edges: Optional[int] = None,
                 feat_mat: Optional[np.ndarray] = None, edge_index: Optional[np.ndarray] = None,
        ...
        if adj_mat is None: #This statement allows for outputting the desired graph struct for non-random or non-synthetic data as well
            if sampling_mode == 'binomial': #THESE TWO METHODS CREATE RANDOMLY SAMPLED
                #ADJACENCY MATRICES ACCORDINGLY-SEE BELOW
    ...
    #@cached_property 
    def neighbors(self): #RETURN ARRAY OF LISTS WHERE EACH LIST IS INDICES OF THE NODE CRSP.
        if self.synthetic:
            #TO THAT LISTS INDEX IN THE ARRAY
            neighbors = [[] for i in range(self.num_nodes)]
            for node in range(self.num_nodes):
                neighbors[node] = list(np.where(self.adj_mat[node, :] > 0)[0])
            return neighbors
        else:
            neighbors = [[] for i in range(len(self.adj_mat))]
            for node in range(len(self.adj_mat)):
                neighbors[node] = list(np.where(self.adj_mat[node, :] > 0)[0])
            return neighbors


    #@cached_property #EACH NODE FEATURES IS NORMALIZED BY SUM OF NEIGHBOURUNG NODIE FEATURES
    def feat_mat_aggr_normed(self): #matrix of \bar h_u
        if self.synthetic:
            ...
            normed_feats = np.zeros((len(self.adj_mat), self.dim_feats))
            neighbors = self.neighbors()
            for node in range(len(self.adj_mat)):
                sum_feats = np.sum(self.feat_mat[neighbors[node]], axis=0)
                normed_feats[node] = sum_feats / np.linalg.norm(sum_feats)
    ...
    def sample_uniform_graph(self, num_edges: int):
        ...
        all_edges = list(combinations(range(self.num_nodes), 2)) #RETURNS ALL POSSIBLE EDGES
        #TO PICK RANDOMYL AND UNIFORMLY FROM
    ...
    def sample_features(self): #SAMPLING GAUSSIAN DISTRIBUTED NODE FEATURES IF NONE AVAILABLE
...
def generate_graph_data(data_size: int, num_nodes: Optional[int], dim_feats: int,
    ...
    # if num_nodes is an int, repeat it' as many times as the data size
    ...
    return data #LIST OF DATA_SIZE MANY SYNTHETIC GRAPHS
